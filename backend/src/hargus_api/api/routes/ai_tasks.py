...
def get_ai_task_service(settings: Settings = Depends(get_settings)) -> AiTaskService:
    return AiTaskService(settings)


@router.get("", response_model=list[AiTaskRecord])
async def list_ai_tasks(service: AiTaskService = Depends(get_ai_task_service)) -> list[AiTaskRecord]:
...
async def get_ai_task(task_id: str, service: AiTaskService = Depends(get_ai_task_service)) -> AiTaskRecord:
