...
from hargus_api.schemas.domain import CandidateListResponse, PaginationMeta, Vacancy, VacancyListResponse
...
async def get_vacancies() -> VacancyListResponse:
    ...
    return VacancyListResponse(items=items, meta=PaginationMeta(total=len(items)))


@router.get("/{vacancy_id}", response_model=Vacancy)
async def get_vacancy_by_id(vacancy_id: str) -> Vacancy:
...
async def get_vacancy_candidates(vacancy_id: str) -> CandidateListResponse:
    vacancy = get_vacancy(vacancy_id)
    if vacancy is None:
        raise HTTPException(status_code=404, detail="Vacancy not found")
